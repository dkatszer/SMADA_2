...
//				points[x][y].addNeighbor(points[x - 1][y - 1]);
	...
	private void calculateField(){
		List<Point> exitPoints = findExitPoints();
		exitPoints.forEach(exitPoint -> exitPoint.staticField = 0);

		Queue<Point> toCheck = initToCheckQueue(exitPoints);
		while(!toCheck.isEmpty()){
			Point pointToCheck = toCheck.remove();
			if(pointToCheck.calcStaticField()){
				toCheck.addAll(pointToCheck.neighbors);
			}
		}
	}

	private Queue<Point> initToCheckQueue(List<Point> exitPoints) {
		return exitPoints.stream().flatMap(point -> point.neighbors.stream()).collect(Collectors.toCollection(LinkedList::new));
	}

	private List<Point> findExitPoints() {
		List<Point> exitPoints = new ArrayList<>();
		for (int x = 0; x < points.length; ++x) {
			for (int y = 0; y < points[x].length; ++y) {
				Point point = points[x][y];
				if(point.type == 2){ //for each ext
